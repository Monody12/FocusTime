...
## 35. 新客户端同步失败：旧服务器拒绝备忘录同步表

**问题现象**：客户端加入备忘录功能后，登录正常但同步失败。第一次同步报"无效的数据表"，之后每次同步都失败，任务数据也无法同步。

**根因**：新客户端把 8 张备忘录表一并放进同步负载（上传负载和下载请求），而线上服务器还是旧版本，`ALL_TABLES` 白名单没有这些表，`POST /api/sync` 直接返回 400 `无效的数据表: memos`。整个同步请求被拒绝，导致连任务数据都无法同步——单个不认识的表拖垮了整次同步。

**方案**：
1. 新版服务器在 `/api/health` 返回能力标志 `memoSync: true`；旧服务器没有该字段。
2. 客户端登录成功时探测 `/api/health` 并持久化能力结论；`serverSupportsMemoSync` 为 false 时，同步负载自动剔除全部备忘录表。
3. 兜底：同步收到 400 且错误消息包含备忘录表名时，自动去掉备忘录表重试一次并记住结论，避免用户卡死在失败循环。
4. 备忘录使用独立水位线 `memoLastSyncTime`：降级期间任务水位线正常推进，备忘录水位线不前进，服务器升级重新登录后，降级期间的备忘录变更仍能完整补传。
5. 客户端启动时若缓存了"不支持"结论会重新探测，服务器升级后自动恢复备忘录同步。

**教训**：
- 客户端扩展同步协议时，必须假设线上服务器可能长期停留在旧版本；任何一张新表都不能成为整次同步的硬依赖。
- 多域数据（任务/备忘录）共享一条水位线时，降级期间被跳过的域必须使用独立水位线，否则跳过窗口内的本地变更会被永久漏传。
- 能力探测（health 端点 + 标志位）+ 400 自动降级 + 持久化结论，三层一起才能保证用户在服务器升级前后无感迁移。

## 36. 设备本地状态被当设置同步，备忘录跨端互相不可见

**问题现象**：Web 端创建备忘录同步成功，另一浏览器可见，但旧版升级上来的 Windows 客户端看不到；Windows 客户端创建的备忘录同步"成功"后 Web 端也看不到。任务同步一切正常，仅备忘录在部分设备上单向失联。

**根因**：设备本地状态键 `serverSupportsMemoSync`（服务器能力探测结论）、`memoLastSyncTime`（备忘录独立水位线）没有被加入设置的同步排除清单，被当成普通设置参与跨设备同步。完整事故链：
1. 服务器还是旧版时，客户端探测不到 `memoSync`，缓存 `serverSupportsMemoSync='false'` 并作为普通设置上传到服务器；
2. 服务器升级后，这条 `'false'` 记录随设置下发到所有设备；旧版客户端虽然启动时会重新探测，但每次设置下载又把本地结论覆盖回 `'false'`，探测成果被反复冲掉；
3. 标志为 false 时上传负载剔除备忘录表、下载请求不包含备忘录表，该设备的备忘录双向静默；
4. 降级窗口期共享下载游标照常推进，已经越过窗口期服务器上的备忘录变更，即使标志恢复，错过窗口的备忘录也永远拉不到（游标污染）。

**方案**：
1. 客户端上传排除清单与下载忽略清单同时加入 `serverSupportsMemoSync`、`memoLastSyncTime`、`memoServerSyncCursor`（设备本地状态绝不跨设备同步）；
2. 每次同步前若缓存为"不支持"则主动探测一次 `/api/health`，`false→true` 恢复时重置共享下载游标与备忘录水位线为 0，靠 LWW 合并做一次全量补拉；
3. 服务端把这三个键列入摄入拒绝清单（`DEVICE_LOCAL_SETTING_KEYS`），旧客户端上传直接丢弃，防止再次污染；
4. 服务端启动时清理 `sync_records` 中的历史污染记录（`purgeDeviceLocalSettingRecords`），未升级的旧客户端失去污染源后靠自身启动探测即可自愈。

**教训**：
- 同步排除清单是同步协议的安全边界：任何"本机探测结论/本机水位线"性质的设置，新增时必须第一时间加入排除清单，否则会被协议当作数据传播。
- 排查"部分设备失联"类问题时，先查该设备本地数据库里的关键标志位和水位线，再对比服务器上的记录——污染源往往在另一台设备的历史状态里。
- 客户端修排除清单只能防新增污染；要让存量旧版本自愈，需要服务端拒绝摄入 + 启动清理双管齐下。

## 37. 字体子集缺字：Firefox 显示"备□录"豆腐块

**问题现象**：备忘录功能上线后，标题"备忘录"的"忘"字在 Chrome 和 Windows 客户端显示正常，Firefox 中显示为方框（U+FFFD 风格豆腐块）。

**根因**：Web 端中文显示依赖 `assets/fonts/FocusNotoSansSC-UI.ttf` 子集字体，由 `tool/setup_ui_font.sh` 扫描 `lib`、`server/src` 的全部非 ASCII 字符生成。备忘录新界面引入的"忘"等 6 个汉字（忘剔愈曾污越）在子集生成之后才进入代码，线上字体没有它们。CanvasKit 渲染下 Chrome 能按字符回退到系统字体，Firefox 对应用字体的回退更严格，缺字直接渲染豆腐块。注意两点：
- Flutter Web 引擎实际通过 `assets/` + FontManifest 资产路径加载字体（如 `assets/assets/fonts/FocusNotoSansSC-UI.ttf`），手工 curl 单层路径返回 404 不代表字体缺失，排查时要用引擎真实请求路径；
- `web/index.html` 里的 `@font-face` 只服务于加载页（FocusLoaderSC0-8），与运行时字体无关。

**方案**：
1. 重新运行字体子集脚本生成包含全部在用汉字的字体并随版本发布；
2. `tool/setup_ui_font.sh` 完成 Windows（Git Bash）兼容（venv 脚本目录探测、`install` 改 `mkdir/cp`），开发机可直接重新生成，新功能上线的字不再依赖"记得跑脚本"。
...
## 39. 升级时序盲区：探测自愈抢先于版本升级，备忘录历史被游标永久跳过

**问题现象**：v1.7.5 修复发布后，用户升级 Windows 客户端并同步，Web 端前一天创建的两篇备忘录仍然拉不到；当天新建的备忘录却能正常双向同步。

**根因**：v1.7.5 的「false→true 重置游标补拉」只保护发生在 v1.7.5 客户端内的标志切换。存量客户端在升级前就由 v1.7.4 的启动探测把标志自愈为 true（v1.7.4 没有重置逻辑），且当时恰逢一篇新备忘录刚创建被正常增量拉到，掩盖了缺口；等升级到 v1.7.5 时标志已是 true，重置分支永远不触发，共享下载游标早已越过降级窗口期的备忘录变更。用双端数据实锤：客户端游标恰好等于服务端 `MAX(server_updated_at)`，缺失备忘录的 `server_updated_at` 全部小于该游标。

**方案**：备忘录获得独立下载游标 `memoServerSyncCursor`（该键在 v1.7.5 已加入全部排除清单）：
1. 下载请求取 `min(共享游标, 备忘录游标)`：窗口期内备忘录游标停在原点，窗口结束后的请求自动回退到缺口起点开始补拉，完全不依赖能力标志的切换时机；
2. 备忘录下载游标只在能力标志为 true 时推进，与上传水位线同一原则；
3. 存量安装该键缺失按 0 处理：升级后首次同步自动做一次全量补拉（LWW 合并，无覆盖风险），一次性修复所有"自愈早于升级"的客户端；
4. 对已发布存量 bug 配合服务端数据侧补救：把备忘录记录的 `server_updated_at` 提升到全体游标之上，旧客户端不升级、点一次同步即可增量拉到缺失记录。

**教训**：
- 修复"状态机转换"类 bug 时，必须检查存量设备是否已经走过该转换——补丁只保护未来的转换，救不了已处于错误稳态的设备。
- 多域共享一个下载游标时，任何域的同步被跳过，共享游标就会越过该域历史。正确设计是每个域独立游标、请求取最小值，让缺口自愈。
- 服务端提升 `server_updated_at` 是让存量旧客户端立即自愈的通用手段，不必等待全量升级；注意保持游标全局单调（取 MAX+1 起步）。

## 40. Flutter Web 字体回退清单必须随引擎版本同步

**问题现象**：Firefox 在备忘录正文中显示方框字，Chrome 因命中简体分片暂时未暴露。

**根因**：Flutter 3.44.7 的 NotoFont 回退清单扩展到 724 个分片，而项目脚本仍只下载四个家族，繁体、韩文及小语种分片请求返回 404。

**方案**：脚本直接解析 SDK 中全部 `NotoFont` 条目并下载，校验数量下限 500；Web 提醒发布时同时更新静态镜像。CanvasKit 不经过浏览器系统字体栈，不能通过取消应用字体解决缺字。

**教训**：Flutter 升级后必须重新生成并核对字体回退资源，Firefox 应作为缺字回归验证浏览器。

## 41. Web 任务提醒不应复用专注完成的循环响铃

**问题现象**：Web 任务提醒持续循环播放内置 alarm.wav，页面没有停止入口。

**根因**：任务调度传入 `duration=long`，触发循环播放；Web 又缺少系统通知分支。

**方案**：Web 提醒强制单次音频，并调用 Notification API 借用浏览器/操作系统通知音；循环响铃仅保留给有明确停止操作的专注完成场景。

**教训**：不同业务语义不能共享“持续时间”默认值；浏览器通知权限失败时必须保证页内提示仍可结束。

## 42. 自动保存与版本历史应分层设计

**问题现象**：备忘录自动保存成功后版本历史仍为空；若每次自动保存都生成版本，短防抖又会迅速挤掉有意义的恢复点。

**根因**：旧实现把“持久化当前内容”和“创建历史快照”绑定成单一布尔开关，自动保存为了控制数量完全关闭快照，没有独立保留策略，也没有浏览器异常退出草稿。

**方案**：`memo_versions` 增加 `source` 字段并将数据库版本升到 15；手动版本保留 20 个，自动版本至少间隔 5 分钟并独立保留 10 个；离开编辑器时补自动恢复点；自动保存支持开关和延迟设置；Web 使用 localStorage、原生端使用设备偏好保存恢复草稿，隐私内容写入前加密。

**教训**：当前文档、版本快照和崩溃恢复草稿是三层不同数据，必须分别设置写入频率、保留上限和安全边界。

*最后更新日期：2026-08-30*

---

## 43. 备忘录跨端冲突与渲染缓存必须覆盖真实交互状态

### 43.1 问题现象
- 远端同步更新到达正在编辑的备忘录时，需要避免无提示覆盖本地内容，并允许用户比较后选择版本。
- “保留本地”在本地正文已经保存、编辑器不再标记为脏时会被普通保存短路，按钮看似可用但实际不写回。
- Markdown 代码块虽然在初次渲染时颜色正确，但应用运行中切换深浅主题或改变窗口宽度后，缓存的旧组件仍沿用原配色与布局。

### 43.2 根因
1. 数据库同步与编辑器状态属于两套时间线：远端行已经写入数据库，但编辑器控制器仍保留本地文本；普通的 `_isDirty` 只能比较控制器与编辑器初始值，不能表达“必须覆盖数据库中的远端行”。
2. Markdown 解析结果依赖正文、主题亮度和窄屏布局，旧缓存键却只有正文内容。
3. 固定横向排列的冲突按钮和移动端顶栏没有为 390px 等窄视口预留换行空间。

### 43.3 方案
1. 远端正文覆盖前自动写入独立 `conflict` 版本池；编辑器发现外部更新时间变化后暂停替换，提供差异比较、采用远端和保留本地。
2. “保留本地”使用明确的强制保存路径，在覆盖数据库远端内容前把远端状态再写入冲突历史，不能依赖普通脏状态判断。
3. Markdown 缓存键同时包含正文、主题亮度和窄屏状态；高亮与普通代码块都显式设置浅色 `#383A42`、深色 `#ABB2BF` 正文色。
4. 窄屏冲突操作使用可换行布局，备忘录页次要顶栏操作收进菜单；新增组件测试验证主题切换后的最终颜色。

### 43.4 教训
- 同步冲突按钮必须按数据库当前值和编辑器当前值设计，不能复用只面向本地输入的“是否修改”判定。
- UI 缓存键必须覆盖构建结果依赖的全部环境变量；只按业务数据缓存会让主题、语言或断点变化失效。
- 响应式验收应至少包含桌面和真实手机宽度，并实际触发最长提示文案与完整操作组。
...
**问题现象**：备忘录文件夹、标签和排序按钮位于横向滚动区域内，菜单虽然可以打开，但使用外层 `Builder` 的上下文计算位置时，锚点可能落在整行而不是被点击的按钮上；数据库单测也无法证明用户能够从界面完成排序。

**根因**：`BuildContext.findRenderObject()` 返回该上下文对应的最近渲染对象。组件型 `Builder` 本身没有独立渲染对象，不能作为按钮几何位置的可靠来源。

**方案**：为三个筛选按钮保留持久 `GlobalKey`，从实际 `Semantics` 按钮节点取得 `RenderBox` 后调用 `showMenu`；新增 Widget 交互测试，实际打开菜单、确认六种排序、选择名称升序、验证列表顺序并检查偏好写入。
...
## 46. v1.8.3 隐私元数据、附件删除和生命周期回归

### 46.1 问题现象

加密标题的私密备忘录在创建、编辑或跨端同步后可能残留文件夹/标签关系；旧版私密负载还可能在解锁后标题前多出换行。附件删除此前只删除本地记录，远端对象和分享链接仍可能保留；Web 提醒触发后 `reminderAt` 不清理，草稿在退出或异步保存竞态下也存在丢失风险。

### 46.2 根因

隐私字段由多个入口分别编码和写入，数据库迁移、同步入库和 Provider 展示没有共享同一契约。附件删除没有贯通对象存储、分享撤销、本地文件和 SQLite 软删除四层。草稿写入是异步 fire-and-forget，保存完成时无条件把编辑器标记为干净，覆盖了保存期间的新输入。

### 46.3 方案

1. 用 `title\u0000body` 统一私密负载，读取兼容 v1.7.0-v1.8.2 的额外换行；数据库 Schema 16 在创建备忘录表之后清理历史文件夹/标签关系，同步入库再次执行相同约束，Provider 解锁后只注入正文和标题。
2. 删除附件先确认正文引用，再删除远端对象并由服务端撤销分享，原生端清理本地文件，最后更新附件 `deleted=1`；远端删除保持幂等，Web 跳过不存在的本地文件层。
3. Web/Windows 提醒触发后清除 `reminderAt` 并触发后台同步，失败通过应用级 SnackBar 反馈。编辑器后台、退出和保存前冲刷草稿，草稿写入串行化；只有保存快照仍是当前内容时才清理草稿。
4. Web 提醒 Overlay 始终返回 `Positioned`，并用松约束 Stack 回归测试验证旧的零尺寸非定位子项确实导致 `0×0`。

### 46.4 教训

- 端到端隐私约束必须在创建、数据库、同步和展示四层都执行，不能只修 UI 入口。
- 删除闭环必须覆盖所有持久化层和分享状态，远端失败时要保留可重试的本地记录并反馈用户。
- 异步保存必须比较快照与当前编辑内容，不能用“请求成功”直接推导“编辑器已无修改”。
- 布局回归测试要放在松约束父级中，先证明测试能复现旧 bug，再证明新实现通过。

*最后更新日期：2026-08-31*

---

## 47. 手动排序与备忘录快捷键必须覆盖真实手势和筛选状态

### 47.1 问题现象

任务清单拖动后立即回到原位置，移动端有时完全无法开始排序。备忘录虽然默认字段是阅读模式，但曾经进入编辑模式后，下一次点开仍会恢复编辑；在搜索框中快速新建时还可能选中新 ID 却显示空白编辑区。

### 47.2 根因

1. 任务 Provider 的乐观更新只改变列表位置，没有更新任务对象的 `sortOrder`；页面按手动模式再次排序时使用旧值，把列表排回原序。数据库又把任务顺序和同步字段版本拆成两次提交。
2. 每个任务项同时注册跨清单 `LongPressDraggable` 和清单内延迟拖动，移动端两个长按识别器竞争同一手势。
3. 备忘录页面持久化了上次阅读/编辑模式，点击现有条目时没有明确重置为阅读。搜索状态下创建的新备忘录不匹配旧查询，Provider 刷新后无法为编辑器提供该行。
4. `TextField` 会接管部分键盘事件，只在外层放置快捷键作用域不能保证标题、正文和搜索框中的行为一致。

### 47.3 方案

1. 乐观重排同时写入连续 `sortOrder` 并立即切换手动模式；数据库在同一事务内更新顺序和 `sync_field_versions`，失败恢复旧状态。
2. 任务项提供独立排序手柄；桌面端保留非手柄区域的跨清单长按拖动，移动端通过操作菜单移动清单，不再注册冲突的长按拖动。
3. 点击已有备忘录强制进入阅读模式且不再持久化阅读/编辑状态；新建仍直接进入编辑。新建前先清空搜索并等待 Provider 恢复未筛选视图。
4. 备忘录页面和编辑器在生命周期内注册并移除 `HardwareKeyboard` 处理器，统一实现保存、搜索、新建、粘贴和返回；弹窗路由位于前台时不拦截快捷键。

### 47.4 教训

- 乐观更新必须更新后续排序依赖的字段，不能只调整容器顺序。
- 同一触点不要注册两个长按拖动语义；排序与跨容器移动应有清晰、独立的入口。
- “默认模式”应由打开动作明确决定，而不是依赖一次初始化值。
- 创建并选中记录前要确认当前筛选视图能返回该记录，否则会出现有 ID、无数据的空白详情页。

*最后更新日期：2026-09-03*

---

## 48. 生产部署必须锁定依赖与 Node 原生模块 ABI

**问题现象**：全新生产工作树运行服务端测试时找不到 `tsc`；锁文件要求不存在的 `etag@1.8.2`；非交互 SSH 默认 Node 20，而 PM2 服务使用 Node 22，`better-sqlite3` 因 ABI 115/127 不匹配无法加载。

**根因**：部署脚本假设 `node_modules` 已存在，只复制源码后直接构建；同时使用 PATH 中的 `npm/node`，没有遵循 `ecosystem.config.js` 固定的解释器。无效锁文件被已有 `node_modules` 掩盖，普通 `npm install --package-lock-only` 也没有纠正该节点。

**方案**：锁文件恢复为注册表存在的 `etag@1.8.1` 并用干净 `npm ci` 验证；脚本从 PM2 配置读取 Node 22 解释器，将同目录 npm 用于本地测试和生产更新，两处均先执行 `npm ci`；部署验收额外确认 PM2 的 `node_interpreter` 与配置一致。

**教训**：发布脚本必须能从无依赖缓存的干净工作树运行；含原生模块的 Node 服务不能只比较 Node 版本字符串，构建、测试和 PM2 运行必须使用同一个解释器路径。
