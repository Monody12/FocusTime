...
class _CalendarPageState extends ConsumerState<CalendarPage> {
  ...
  Future<void> _loadMonthData() async {
    ...
    for (final session in sessions) {
      final date = DateTime.fromMillisecondsSinceEpoch(session['startedAt'] as int).toIso8601String().split('T')[0];
      final existing = stats[date] ?? {'focusMinutes': 0, 'taskCount': 0, 'completedCount': 0, 'recurringCount': 0};
      existing['focusMinutes'] = (existing['focusMinutes'] as int) + ((session['durationSeconds'] as int) / 60).floor();
      stats[date] = existing;
    }

    // Recurring tasks
    final recurring = allTasks.where((t) => t['recurrenceConfig'] != null && t['dueDate'] != null).toList();
    for (final task in recurring) {
      final dates = getRecurrenceDatesInRange(
        RecurrenceConfig.fromJson(task['recurrenceConfig'] as Map<String, dynamic>),
      ...
      for (final date in dates) {
        ...
        final existing = stats[dateStr] ?? {'focusMinutes': 0, 'taskCount': 0, 'completedCount': 0, 'recurringCount': 0};
  ...
  Future<void> _loadDateDetail() async {
    ...
    final dayTasks = allTasks.where((t) => t['dueDate'] == _selectedDate).toList();
    ...
    for (final task in _recurringTasks) {
      ...
      if (dates.isNotEmpty) {
        ...
        final taskCompletions = await AppDatabase.getRecurrenceCompletionsByDateRange(
          task['id'] as String,
          _selectedDate,
          _selectedDate,
        );
        ...
        Padding(
            ...
            child: Column(
              ...
              children: [
                        ...
                        child: Row(
                          children: [
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                color: isDark ? AppColors.darkText : AppColors.lightText,
                                decoration: _selectedDateCompletions.contains(task['id'])
                ...
                if (_selectedDateSessions.isEmpty && _recurringTasks.isEmpty && _selectedDateTasks.isEmpty)
                  Padding(
