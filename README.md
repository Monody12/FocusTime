...
### 🛠️ 优化与修复
- **设置页交互优化**：彻底解决了同步服务器配置区域的 Tab 焦点导航问题。
    - **焦点隔离**：引入 `FocusTraversalGroup` 将登录表单与全局焦点环境隔离，防止光标意外跳至侧边栏。
    - **精准路径控制**：通过自定义 `FocusNode` 拦截硬件按键，实现了“服务器地址 → 用户名 → 密码 → 登录/注册”的线性跳转路径。
    - **按钮交互增强**：优化了自定义按钮组件对焦点的响应，确保全键盘操作体验流畅。
